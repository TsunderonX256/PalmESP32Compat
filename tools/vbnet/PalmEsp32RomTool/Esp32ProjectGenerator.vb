...
Public NotInheritable Class Esp32ProjectGenerator
    ...
    Private Shared Function RenderPlatformIo(request As Esp32ProjectRequest) As String
        ...
        Dim isQemu = IsQemuHardwareProfile(request.HardwareProfile)
        Dim qemuFlag = If(isQemu, "1", "0")
        Dim extraBoardLines = If(isQemu,
"board_build.flash_mode = dio
board_build.psram_type = disabled
", "board_build.arduino.memory_type = qio_opi
")
        Dim psramFlag = If(isQemu, "", "  -DBOARD_HAS_PSRAM" & Environment.NewLine)
...
"
    ...
    Private Shared Function IsQemuHardwareProfile(hardwareProfile As String) As Boolean
        Return hardwareProfile.IndexOf("qemu", StringComparison.OrdinalIgnoreCase) >= 0 OrElse
            hardwareProfile.IndexOf("headless", StringComparison.OrdinalIgnoreCase) >= 0
    End Function

    Private Shared Function RenderConfig(request As Esp32ProjectRequest) As String
        Dim qemuFlag = If(IsQemuHardwareProfile(request.HardwareProfile), "1", "0")
        Return $"#pragma once
...
#define PALM_COMPAT_QEMU_HEADLESS {qemuFlag}
...
static constexpr uint16_t kMemoDoneButtonId = 9002u;
static constexpr uint16_t kMemoCancelButtonId = 9003u;
...
struct FakePalmFormObject
{
    ...
    uint16_t popupControlId;
    uint16_t popupListId;
    uint16_t linkedObjectId;
    char label[16];
};
...
static bool fakeFormDecodePopupMetadata(const PalmGeneratedOverlayResource& resource, uint32_t objectOffset, uint16_t& controlId, uint16_t& listId)
{
    if (resource.bytes == nullptr || objectOffset + 4u > resource.size)
    {
        return false;
    }

    controlId = readOverlayU16BE(resource, objectOffset + 0u);
    listId = readOverlayU16BE(resource, objectOffset + 2u);
    return controlId != 0u && controlId != 0xffffu && listId != 0u && listId != 0xffffu;
}

static bool fakeFormLinkPopupMetadata(uint16_t controlId, uint16_t listId)
{
    bool linked = false;
    for (uint8_t i = 0; i < gFakeFormObjectCount; ++i)
    {
        ...
        if (object.objectId == controlId)
        {
            object.popupControlId = controlId;
            object.popupListId = listId;
            object.linkedObjectId = listId;
            fakeControlSyncMemory(object);
            linked = true;
        }
        else if (object.objectId == listId)
        {
            object.popupControlId = controlId;
            object.popupListId = listId;
            object.linkedObjectId = controlId;
            if (controlId == kPalmMemoCategoryTriggerId && listId == kPalmMemoCategoryListId)
            {
                palmDisplayPalmUiSetCategoryListBounds(object.x, object.y, object.w, object.h);
            }
            fakeControlSyncMemory(object);
            linked = true;
        }
...
static void fakeFormRefreshPopupMetadataLinks()
{
    for (uint8_t i = 0; i < gFakeFormObjectCount; ++i)
    {
        const FakePalmFormObject& object = gFakeFormObjects[i];
        if (object.used && object.popupControlId != 0u && object.popupListId != 0u)
        {
            fakeFormLinkPopupMetadata(object.popupControlId, object.popupListId);
        }
    }
}
...
static uint16_t fakeFormAddObjectExplicit(uint16_t objectId, uint16_t kind, int16_t x, int16_t y, int16_t w, int16_t h, int16_t tableRows, int16_t tableColumns, const char* label, uint8_t formObjectIndex = 0xffu, uint16_t style = 0u, uint16_t group = 0u, uint16_t fontId = 0u)
{
    ...
    object.kind = kind;
    object.ptr = gFakeActiveFormPtr + 32u + static_cast<uint32_t>(index) * kFakeFormObjectRecordSize;
    object.value = 0;
    object.enabled = true;
    object.usable = true;
    object.visible = true;
...
static uint8_t fakeFormScoreObjectList(const PalmGeneratedOverlayResource& resource, uint32_t objectListOffset, uint16_t objectCount)
{
    ...
    for (uint16_t i = 0; i < objectCount; ++i)
    {
        ...
        if (objectType == 10u && fakeFormDecodePopupMetadata(resource, objectOffset, popupControlId, popupListId))
        {
            ++score;
        }
...
static bool fakeFormDecodeFromResource(const PalmGeneratedOverlayResource& resource)
{
    ...
    for (uint16_t i = 0; i < objectCount; ++i)
    {
        ...
        if (!fakeFormDecodeObjectBounds(resource, objectType, objectOffset, objectId, x, y, w, h, tableRows, tableColumns, style, group, fontId, label, sizeof(label)))
        {
            ...
            if (objectType == 10u && fakeFormDecodePopupMetadata(resource, objectOffset, popupControlId, popupListId))
            {
                const bool popupLinked = fakeFormLinkPopupMetadata(popupControlId, popupListId);
                ++decodedCount;
                Serial.printf(""    tFRM object %u type=%u popup control=%u list=%u linked=%s\n"",
                    static_cast<unsigned>(i),
                    static_cast<unsigned>(objectType),
                    static_cast<unsigned>(popupControlId),
                    static_cast<unsigned>(popupListId),
                    popupLinked ? ""yes"" : ""pending"");
                continue;
            }
...
static uint32_t fakeFormInit(uint16_t formId)
{
    ...
    {
        gFakeActiveFormHandle = tinyHandleAlloc(1024u);
...
void palmDisplayPalmUiSetCategoryListBounds(int16_t x, int16_t y, int16_t w, int16_t h);
...
static bool gHeadlessDisplayReady = false;
...
static constexpr uint16_t kMemoDoneButtonId = 9002;
static constexpr uint16_t kMemoCancelButtonId = 9003;
...
static PalmUiObjectBounds gPalmUiCategoryListBounds = {false, 0xffffu, 88, 14, 72, 0};
...
static PalmUiObjectBounds palmUiCategoryPopupBounds();
...
void palmDisplayBacklightOff()
{
#if PALM_COMPAT_QEMU_HEADLESS
    Serial.println(""LCD headless mode: backlight disabled"");
#else
...
{
    const PalmUiObjectBounds bounds = palmUiCategoryPopupBounds();
    const int x = bounds.x;
    const int y = bounds.y;
    const int w = bounds.w;
    const int rowH = 11;
    const int h = bounds.h > 0 ? bounds.h : rowH * 2 + 3;
...
{
    const PalmUiObjectBounds bounds = palmUiCategoryPopupBounds();
    const int listX = bounds.x;
    const int listY = bounds.y;
    const int w = bounds.w;
    const int rowH = 11;
    const int h = bounds.h > 0 ? bounds.h : rowH * 2 + 3;
    if (!inRect(x, y, listX, listY, w, h))
    {
        return -1;
    }
...
void palmDisplayPalmUiSetCategoryListBounds(int16_t x, int16_t y, int16_t w, int16_t h)
{
    ...
    gPalmUiCategoryListBounds.used = true;
    gPalmUiCategoryListBounds.x = x;
    gPalmUiCategoryListBounds.y = y;
    gPalmUiCategoryListBounds.w = w;
    gPalmUiCategoryListBounds.h = h;
}

static PalmUiObjectBounds palmUiCategoryPopupBounds()
{
    const int rowH = 11;
    PalmUiObjectBounds bounds = gPalmUiCategoryListBounds;
    if (!bounds.used)
    {
        const PalmUiObjectBounds trigger = gPalmUiCategoryBounds;
        bounds.used = true;
        bounds.w = trigger.w < 52 ? 52 : trigger.w;
        bounds.x = trigger.x + trigger.w - bounds.w;
        bounds.y = trigger.y + trigger.h + 1;
        bounds.h = rowH * 2 + 3;
        return bounds;
    }
    ...
    if (bounds.y <= gPalmUiCategoryBounds.y)
    {
        bounds.y = gPalmUiCategoryBounds.y + gPalmUiCategoryBounds.h + 1;
    }
    if (bounds.x < 0)
    {
        bounds.x = 0;
    }
    if (bounds.x + bounds.w > kPalmLcdW)
    {
        bounds.x = static_cast<int16_t>(kPalmLcdW - bounds.w);
    }
    if (bounds.y + bounds.h > kPalmLcdH)
    {
        bounds.y = static_cast<int16_t>(kPalmLcdH - bounds.h);
    }
    return bounds;
}
...
bool palmDisplayBegin()
{
    if (gPanel != nullptr || gHeadlessDisplayReady)
    {
...
#if PALM_COMPAT_QEMU_HEADLESS
    gHeadlessDisplayReady = true;
    Serial.println(""LCD headless framebuffer initialized for QEMU"");
    return true;
#else
